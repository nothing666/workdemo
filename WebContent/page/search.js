...
function showTravelRecord(){
	...
	$('#ccjl').tablegrid({
		url:'/rail/travelRecord/queryTravelPage.htm',
		 dataType:'json',
         colModel:[{
            display:'乘车人姓名',
            name:'passenger',
...
function showKp(smallTitleCode){
	...
	if(smallTitleCode == "dprsfxx"){	
		...
		$('#'+smallTitleCode).objectgrid({
			 url:'/rail/travelRecord/findPassengerIdentityPage.htm',
			 dataType:'json',
			 checkbox:true,
			 usepager:true,
			 width:575,
...
//同邮箱的订票分析(注：此处的id已经去掉了@.等特殊字符,需根据id找到row,在找到对应的cell数据)
function mailAnalysis(id){
	if(id){
		var mailStr = $('#ul_dprsfxx_'+id).parent().find(".wordWrapper span").html();
		window.open("echart/getBarChart?mail="+mailStr);
	}
}
...
function addAnalyse(e){
	if($(e.target).html()=='全息展示'){
		...
		var imgObj = $('#'+buttonId).parent().children('ul').find('img');
		var jsonStr = imgObj.attr('tagname');
		
		
		var dataObject = JSON.parse(jsonStr);
		var cell = dataObject.cell;
		
		var url =  "page/index.html?name="+cell.name+"&bookingMail="+cell.bookingMail+"&authAccount="+cell.authAccount+
					"&certificateCode="+cell.certificateCode+"&departureStation="+cell.departureStation+
					"&destinationIp="+cell.destinationIp+"&imei="+cell.imei+"&imsi="+cell.imsi+
					"&mobile="+cell.mobile+"&sourceIp="+cell.sourceIp+"&sourceMac="+cell.sourceMac+
					"&terminalStation="+cell.terminalStation+"&station="+cell.station;
...
function addItemHtml(code,value,searchVal){
	...
	if(searchMoreItems.indexOf(code)>-1){
		if(code && searchVal){
			if(searchMoreItems.indexOf(code+':'+searchVal) > -1){
				return "false";
			}
		}
	}
...
function deleteSearch(obj,code){
	...
	var removeItemCode= code;
	var removeItemValue = $(obj).parent().find('span').html().split('：')[1];
	
	var items = $('#searchMoreItems').val().split(',');
	var result = "";
	for(var i = 0 ;i<items.length;i++){
		if(items[i].length>0 && items[i].indexOf(removeItemCode+':'+removeItemValue) == -1){
			result+=(items[i]+',');
		}
	}
	if(result.length<=0){
...
function testNetRelation(){
	
	
	window.open(encodeURI(encodeURI("page/index.html?姓名=张三&pp=234")));
};
