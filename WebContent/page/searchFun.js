...
Zz.search.queryAll= function(param){
	...
	var reslut = buildSearchParam(param,travelParam,bookingParam);
	if(! reslut){
		return ;
	}
...
Zz.search.queryTable= function(param,containerId){
	...
	var reslut = buildSearchParam(param,travelParam,bookingParam,containerId);
	if(! reslut){
		return ;
	}
...
function buildSearchParam(param,travelParam,bookingParam,containerId){
	...
	if(passengerTypeParam.passengerType){
		travelParam.pType = passengerTypeParam.passengerType;
		bookingParam.bType = passengerTypeParam.passengerType;
	...
	var bTimeCheck = timeParam.btime.match(reg);
	var eTimeCheck = timeParam.etime.match(reg);
	if(!bTimeCheck || !eTimeCheck){
		return false;
	}
	...
	if(param.ccxxSecondSearch && containerId && containerId == 'ccxx'){
		...
		travelParam.secondSearch = param.sfxxSecondSearch;
...
Zz.search.getMultiPara = function (searchMoreItems){
	...
	for(var i = 0 ;i<items.length;i++){
		if(items[i].length>0){
			...
			if(query[0] == 'xm'){
				paramsXmStr = paramsXmStr+query[1]+",";	
			}else if(query[0] == 'sjh'){
				paramsSjhStr = paramsSjhStr + query[1] + ",";	
			}else if(query[0] == 'sfzh'){
				paramsSfzhStr = paramsSfzhStr + query[1] + ",";	
			}else if(query[0] == 'dpyx'){
				paramsDpyxStr = paramsDpyxStr + query[1] + ",";	
			}else if(query[0] == 'cc'){
				paramsCcStr = paramsCcStr + query[1] + ",";	
			}else if(query[0] == 'cfd'){
				paramsCfdStr = paramsCfdStr + query[1] + ",";	
			}else if(query[0] == 'mdd'){
				paramsMddStr = paramsMddStr + query[1] + ",";	
			}
		};
	}
	if(paramsXmStr.length > 1){
		paramsXmStr = paramsXmStr.substring(0,paramsXmStr.length-1);
	}
	if(paramsSjhStr.length > 1){
		paramsSjhStr = paramsSjhStr.substring(0,paramsSjhStr.length-1);
	}
	if(paramsSfzhStr.length > 1){
		paramsSfzhStr = paramsSfzhStr.substring(0,paramsSfzhStr.length-1);
	}
	if(paramsDpyxStr.length > 1){
		paramsDpyxStr = paramsDpyxStr.substring(0,paramsDpyxStr.length-1);
	}
	if(paramsCcStr.length > 1){
		paramsCcStr = paramsCcStr.substring(0,paramsCcStr.length-1);
	}
	if(paramsCfdStr.length > 1){
		paramsCfdStr = paramsCfdStr.substring(0,paramsCfdStr.length-1);
	}
	if(paramsMddStr.length > 1){
		paramsMddStr = paramsMddStr.substring(0,paramsMddStr.length-1);
	}
	params['xm'] = paramsXmStr;
	params['sjh'] = paramsSjhStr;
	params['sfzh'] = paramsSfzhStr;
	params['dpyx'] = paramsDpyxStr;
	params['cc'] = paramsCcStr;
	params['cfd'] = paramsCfdStr;
	params['mdd'] = paramsMddStr;
